...
void MPIMergeSortLong::mergesort(std::vector<long>& arr, int nproc)
{
    ...
    std::vector<long> local(work);//local vector for each rank 
    MPI_Scatter(arr.data(),work, MPI_LONG, local.data(), work, MPI_LONG,0, comm);//Scatter equal work to processes
    ...
    while(sizeOfSortedArrays <= work)
    {
        ...
        for(int i =0; i < work; i += merge_work)// loop for every 2 logical arrays to merge
        {
            ...
            std::vector<long> tempVector(work); // temporary vector used to help sort the logical arrays
    ...
    MPI_Gather(local.data(),work,MPI_LONG,arr.data(),work, MPI_LONG,0,comm);//gather back to 0 
    ...
    if(rank ==0)// merges the work each thread did
    {
        ...
        while (sizeOfSortedArrays <= n)
        {
            ...
            for(int i =0; i < n; i += merge_work)// merges the work each thread did
            {
                ...
                std::vector<long> tempVector(n); // temporary vector used to help sort the logical arrays
...
void MPIMergeSortLong::setComm(MPI_Comm &communicator)
{
    MPIMergeSortLong::comm = communicator;
}